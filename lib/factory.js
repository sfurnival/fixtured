...
let _generate = (def) => {
    switch (def.type) {
        case 'string':
            return Generate.String.apply(Generate.String, def.args);

        case 'int':
            return Generate.Integer.apply(Generate.Integer, def.args);

        case 'float':
            return Generate.Float.apply(Generate.Float, def.args);
            
        case 'chance':
            return Generate.$.apply(Generate.$, def.args);
            
        case 'oneof':
            return Generate.OneOf.apply(Generate.OneOf, def.args);
...
FixtureFactory.prototype = {
     ...
     *
     * @param   {mixed}     id      - ID for the copy to by known by.
     * @param   {obj}       obj     - The object to store
     * @return  {mixed}             - The desired fixture copy.
     */
    set: function(id, obj) {
        return this._registry.set(id, obj);
    },
